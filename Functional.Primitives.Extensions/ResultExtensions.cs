...
namespace Functional
{
	...
	public static partial class ResultExtensions
	{
		...
		public static TSuccess ThrowIfFailure<TSuccess, TFailure>(this Result<TSuccess, TFailure> result, Func<TFailure, Exception> exceptionFactory)
			=> result.TryGetValue(out var success, out var failure) ? success : throw exceptionFactory.Invoke(failure);

		public static async Task<TSuccess> ThrowIfFailure<TSuccess, TFailure>(this Task<Result<TSuccess, TFailure>> result, Func<TFailure, Exception> exceptionFactory)
			=> (await result).ThrowIfFailure(exceptionFactory);
	}
}

	...
	public static partial class DiscriminatedUnion
	{
		public static class Implementation
		{
			...
			private static string GetNestedType(INamedTypeSymbol unionType, INamedTypeSymbol nestedType, INamedTypeSymbol[] types)
			{
				...
				builder.AppendLine($"			{{");
				builder.AppendLine($"				// {nestedType.TypeKind}");
				builder.AppendLine($"				if ({lowerName} == null)");
				builder.AppendLine($"					throw new global::System.ArgumentNullException(nameof({lowerName}));");
				builder.AppendLine($"				");
